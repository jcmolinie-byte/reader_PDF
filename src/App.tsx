...
export default function App() {
  ...
  const handleAnalyze = useCallback(async (pdfDoc?: pdfjsLib.PDFDocumentProxy, pagesCount?: number) => {
    ...
    try {
      let fullText = '';
      // Extract text from first few pages for analysis (to avoid huge prompts)
      const pagesToAnalyze = Math.min(count, 5);
      for (let i = 1; i <= pagesToAnalyze; i++) {
        const page = await docToAnalyze.getPage(i);
        const textContent = await page.getTextContent();
    ...
    <div className="min-h-screen bg-[#FDFCFB] text-[#1A1A1A] font-sans selection:bg-orange-100">
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-black/5 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold tracking-tight">AI PDF Highlighter</h1>
            <p className="text-xs text-black/40 font-medium uppercase tracking-wider">Lecteur Intelligent</p>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8">
        ...
        <div className="flex flex-col gap-4">
          {!file ? (
            <div className="aspect-[3/4] w-full border-2 border-dashed border-black/10 rounded-3xl flex flex-col items-center justify-center gap-4 bg-white">
