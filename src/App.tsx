...
export default function App() {
  ...
  const handleAnalyze = useCallback(async (pdfDoc?: pdfjsLib.PDFDocumentProxy, pagesCount?: number) => {
    ...
    try {
      ...
      const pagesToAnalyze = Math.min(count, 2);
  ...
  return (
    <div className="min-h-screen bg-[#FDFCFB] text-[#1A1A1A] font-sans selection:bg-orange-100" onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}>
      {/* Header */}
      ...
      <main className="max-w-7xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8">
        ...
        <div className="flex flex-col gap-4">
          {!file ? (
            <div className={`aspect-[3/4] w-full border-2 border-dashed rounded-3xl flex flex-col items-center justify-center gap-4 bg-white transition-colors ${isDragging ? "border-black/40 bg-black/5" : "border-black/10"}`}>
                  ...
                  <table className="w-full text-sm border-collapse">
                    <thead>
                      <tr className="bg-black/5">
                        <th className="text-left text-[10px] font-bold uppercase tracking-widest text-black/40 px-4 py-2.5 w-2/5">Champ</th>
                        <th className="text-left text-[10px] font-bold uppercase tracking-widest text-black/40 px-4 py-2.5">Valeur</th>
                      </tr>
                    </thead>
                    <tbody>
                      {highlights.map((h, i) => (
                        <tr
                          ...
                          style={{ backgroundColor: i % 2 === 0 ? 'transparent' : 'rgba(0,0,0,0.01)' }}
                        >
                          <td className="px-4 py-3 align-top">
                            <span
                              className="inline-block text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full"
                              style={{ backgroundColor: getCategoryColor(h.category), color: '#1A1A1A' }}
                            >
                              {h.category}
                            </span>
                          </td>
                          <td className="px-4 py-3 align-top font-semibold text-xs leading-snug text-black/80">{h.text}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
